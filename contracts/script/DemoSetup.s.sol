...
contract DemoSetup is Script {
    ...
    function run() external {
        ...
        _phase1bOnboard(deployer, investor);
    ...
    function _phase1Deploy(address deployer) internal {
        ...
        // Central on-chain access manager. Deployer receives OWNER_ROLE.
        protocolRoles = new ProtocolRoles(deployer);
        console.log("ProtocolRoles:", address(protocolRoles));

        // Demo wallet 1 holds all operational roles (single-wallet demo setup).
        protocolRoles.grantRole(protocolRoles.UNDERWRITING_CURATOR_ROLE(), deployer);
        protocolRoles.grantRole(protocolRoles.AUTHORIZED_CEDANT_ROLE(), deployer);
        protocolRoles.grantRole(protocolRoles.PREMIUM_DEPOSITOR_ROLE(), deployer);
        protocolRoles.grantRole(protocolRoles.KYC_OPERATOR_ROLE(), deployer);
        protocolRoles.grantRole(protocolRoles.ALLOCATOR_ROLE(), deployer);
        protocolRoles.grantRole(protocolRoles.SENTINEL_ROLE(), deployer);
        console.log("Roles granted to deployer: CURATOR, CEDANT, PREMIUM_DEPOSITOR, KYC, ALLOCATOR, SENTINEL");
    ...
    // ============================================
    // Phase 1b: KYC onboarding (demo wallets)
    // ============================================
    function _phase1bOnboard(address deployer, address investor) internal {
        // DEMO KYC: ten-year expiry for demo wallets.
        uint64 demoKycExpiry = uint64(block.timestamp + 3650 days);
        compliance.setWhitelist(deployer, true);
        compliance.setKycExpiry(deployer, demoKycExpiry);
        compliance.setWhitelist(investor, true);
        compliance.setKycExpiry(investor, demoKycExpiry);
        console.log("KYC onboarded: deployer + investor");
    }
    ...
    function _setVaultRoles(address deployer) internal {
        // Phase 9.5: legacy oracleReporter/insurerAdmin bindings were removed
        // together with the legacy vault claim triggers. Claims now flow only
        // through the institutional ClaimManager (committee-gated).
        deployer; // silence unused
        console.log("Vault role bindings skipped (legacy claim path removed in Phase 9.5)");
    }
